...
string abspath(const string& path) {
	...
	if (realpath(path.c_str(), resolved_path) == NULL) {
		throw std::runtime_error(
		  "libdivvun: ERROR: realpath(3) failed to resolve '" + path +
...
void PipeSpec::parsePipeSpec(
  const string& dir, pugi::xml_parse_result& result, const string& file) {
	if (result) {
		...
		default_pipe =
		  fromUtf8(doc.child("pipespec").attribute("default-pipe").value());
		for (pugi::xml_node pipeline :
		  doc.child("pipespec").children("pipeline")) {
			const u16string& pipename =
			  fromUtf8(pipeline.attribute("name").value());
			...
			for (const pugi::xml_node& cmd : pipeline.children()) {
				for (const pugi::xml_node& arg : cmd.children()) {
					arg.attribute("n").set_value(
					  pathconcat(dir, arg.attribute("n").value()).c_str());
				}
			}
			pnodes[pipename] = pipeline;
		}
		if (pnodes.find(default_pipe) == pnodes.end()) {
			throw std::runtime_error("libdivvun: ERROR: Couldn't find "
			                         "pipeline with name of default-pipe " +
			                         toUtf8(default_pipe));
		}
	}
	else {
		std::cerr << file << ":" << result.offset
		          << " ERROR: " << result.description() << "\n";
		throw std::runtime_error(
		  "libdivvun: ERROR: Couldn't load the pipespec.xml from archive");
	}
}
...
void validatePipespecCmd(
  const pugi::xml_node& cmd, const std::unordered_map<string, string>& args) {
	const string& name = cmd.name();
	if (name == "tokenise" || name == "tokenize") {
		if (args.size() != 1 || args.find("tokenizer") == args.end()) {
			throw std::runtime_error(
			  "Wrong arguments to <tokenize> command (expected <tokenizer>), "
			  "at byte offset " +
		...
		if (args.size() != 1 || args.find("grammar") == args.end()) {
			throw std::runtime_error("Wrong arguments to <cg> command "
			                         "(expected <grammar>), at byte offset " +
			                         std::to_string(cmd.offset_debug()));
		}
	}
	else if (name == "cgspell") {
		if (args.size() != 2 || args.find("lexicon") == args.end() ||
		    args.find("errmodel") == args.end()) {
			throw std::runtime_error(
			  "Wrong arguments to <cgspell> command (expected <lexicon> and "
			  "<errmodel>), at byte offset " +
			  std::to_string(cmd.offset_debug()));
		}
	}
	else if (name == "mwesplit") {
		if (args.size() != 0) {
			throw std::runtime_error("Wrong arguments to <mwesplit> command "
			                         "(expected none), at byte offset " +
			                         std::to_string(cmd.offset_debug()));
		}
	}
		...
		if (args.size() != 1 || args.find("blanktagger") == args.end()) {
			throw std::runtime_error(
			  "Wrong arguments to <blanktag> command (expected "
			  "<blanktagger>), at byte offset " +
			  std::to_string(cmd.offset_debug()));
		}
	}
	else if (name == "suggest") {
		if (args.size() != 2 || args.find("generator") == args.end() ||
		    args.find("messages") == args.end()) {
			throw std::runtime_error(
			  "Wrong arguments to <suggest> command (expected <generator> and "
			  "<messages>), at byte offset " +
			  std::to_string(cmd.offset_debug()));
		}
	}
	else if (name == "phon") {
		if (args.size() < 1 || args.find("text2ipa") == args.end()) {
			throw std::runtime_error("Wrong arguments to <phon> command "
			                         "(expected <text2ipa>), at byte offset " +
			                         std::to_string(cmd.offset_debug()));
		}
	}
	else if (name == "sh") {
...
std::vector<std::pair<string, string>> toPipeSpecShVector(
  const PipeSpec& spec, const u16string& pipename, bool trace, bool json) {
	...
	for (const pugi::xml_node& cmd : spec.pnodes.at(pipename).children()) {
		...
		if (!prog.empty()) {
			cmds.push_back(std::make_pair(prog, makeDebugSuff(name, args)));
		}
	}
	return cmds;
}

void writePipeSpecSh(const string& specfile, const u16string& pipename,
  bool json, std::ostream& os) {
...
void chmod777(const string& path) {
	...
	if (chmod(path.c_str(), mode) < 0) {
		throw std::runtime_error("libdivvun: ERROR: failed to chmod 777 " +
		                         path + std::strerror(errno));
	}
}

void writePipeSpecShDirOne(
  const std::vector<std::pair<std::string, std::string>>& cmds,
  const string& pipename, const string& modesdir, bool nodebug) {
	...
	for (; i < cmds.size(); ++i) {
		...
		if (!ofs) {
			throw std::runtime_error("libdivvun: ERROR: Couldn't open " +
			                         path + " for writing! " +
			                         std::strerror(errno));
		}
		bool first = true;
...
void writePipeSpecShDir(
  const string& specfile, bool json, const string& modesdir, bool nodebug) {
	const std::unique_ptr<PipeSpec> spec(new PipeSpec(specfile));
	for (const auto& p : spec->pnodes) {
		...
		writePipeSpecShDirOne(toPipeSpecShVector(*spec, p.first, false, json),
		  pipename, modesdir, nodebug);
		if (!nodebug) {
			writePipeSpecShDirOne(
			  toPipeSpecShVector(*spec, p.first, true, json),
			  "trace-" + pipename, modesdir, nodebug);
		}
	}
}
}
