...
const Reading proc_subreading(const string& line, bool generate_all_readings) {
	...
	for (auto& tag : allmatches(tags, CG_TAGS_RE)) { // proc_tags
		...
		if (tag == "&LINK" || tag == "&COERROR" ||
		    tag ==
		      "COERROR") { // &LINK and COERROR kept for backward-compatibility
			r.coerror = true;
		}
...
build_squiggle_replacement(const Reading& r, const ErrId& err_id,
  ...
  const size_t i_right, bool verbose) {
	...
	if (verbose)
		if (r.drop_pre_blank)
			std::cerr << "\033[1;33mr.drop_pre_blank=\t" << r.drop_pre_blank
			          << "\033[0m" << std::endl;
	if (verbose)
	...
	for (size_t i = i_left; i <= i_right; ++i) {
		...
		for (auto& rep : reps) {
			...
			const auto pre_blank =
			  i == i_left || added_before_blank || r.drop_pre_blank ?
			    "" :
...
vector<Err> Suggest::run_errs(std::istream& is) {
	...
	try {
		...
		std::locale mixed(
		  std::locale(""), std::locale::classic(), std::locale::numeric);
