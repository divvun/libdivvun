...
The less central parts need the =&COERROR= tag if all parts are to be underlined as one error.
...
where the verb alone already expresses the concept. This is the case for "tjuedtjelh bæjjese" (verb + adverb) meaning stand.up up. 
With the following rule we first annotate the error and then delete the adverb "bæjjese".


#+begin_src cg
ADD:xsyn-delete-adv-phrasal-verb (&syn-delete-adv-phrasal-verb) TARGET (Adv) IF (0 ("bæjjese")) (-1 ("tjuedtjielidh")) ;

ADDRELATION:xsyn-delete-adv-phrasal-verb (DELETE1) (V &syn-delete-adv-phrasal-verb) TO (1 Adv &syn-delete-adv-phrasal-verb)
#+end_src
...
- =&COERROR= makes a cohort non-central in that error, see [[*Deleting words][Deleting words]].
