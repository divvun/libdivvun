...
the Giellatekno/Divvun Northern Saami package (from
...
Here is the =pipespec.xml= for Northern Saami:
...
the Northern Saami checker, these should appear in
...
(dictionary) FST and error model FST. These are the same format as
the files used by [[https://github.com/hfst/hfst-ospell/][hfst-ospell]].
...
The module adds the tag =<spelled>= to any suggestions. To turn these
into error underlines and actually show the suggestions, add a rule
