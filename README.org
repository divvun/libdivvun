...
Here is the current pipeline definition (=pipespec.xml=) for Northern
Saami:
...
- The main =<cg>= grammar checker module can now add error tags to
  ...
  special tags for deleting words or expanding underlines.
...
In our Northern Saami checker, the
#+begin_src xml
<cg><arg n="grammarchecker.bin"></cg>
#+end_src
...
A real example of this in the Northern Saami checker is the error
...
The =divvun-cgspell= module adds the tag =<spelled>= to any
suggestions. To turn these into error underlines and actually show the
suggestions, add a rule

#+begin_src cg
  ADD (&typo &SUGGESTWF) (<spelled>) ;
#+end_src

to the grammar checker CG. You may of course also =REMOVE= or =SELECT=
such readings first.
...
- =&SUGGEST= on a reading means that =divvun-suggest= should try to
  ...
  generator FST. See [[README.org::*Simple grammarchecker.cg3 rules][Simple grammarchecker.cg3 rules]].
- =&SUGGESTWF= on a reading means that =divvun-suggest= should use the
  ...
  See [[README.org::*Including spelling
   errors][Including spelling errors]].
- =<spelled>= is added by =divvun-cgspell= to any suggestions it
  makes. See [[README.org::*Including spelling
   errors][Including spelling errors]].
- =&LINK= makes a cohort non-central in that error, see [[README.org::*Deleting words][Deleting
  words]].
- =&ADDED= means this cohort was added (typically with =ADDCOHORT=)
  ...
  cohort of the error. See [[README.org::*Adding words][Adding words]].
...
- =LEFT= and =RIGHT= are used to extend the underline to added
  cohorts; see [[README.org::*Adding words][Adding words]].
- =DELETE1= (and =DELETE2= etc.) are used to say that a word in the
  context of this error should be deleted in the suggestion. See [[README.org::*Deleting
   words][Deleting words]].
- =$2= (and =$3= etc.) are used to make wordforms in the context
  ...
  error (so don't add =$1= as a relation). See [[README.org::*Simple grammarchecker.cg3
   rules][Simple grammarchecker.cg3 rules]].
...
- [X] handle [[*sketch for alternative suggestions on the same word][alternative suggestions on the same word]]
