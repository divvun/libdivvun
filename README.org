...
The =divvun-blanktag= program will tag a cohort if it finds a match on
the wordform (including the CG wordform delimiters ="<= and =>"=) plus
surrounding blanks (the start-of-line colon is not included). The
rules file is FST with blank-wordform-blank on the input side, and the
tag on the output-side, typically written in the XFST regex format.

As an example, if the =input.txt= contains
