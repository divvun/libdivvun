...
#+begin_src cg
"<3>"
	"3" Num Arab Sg Loc Attr <W:0.0000000000> @HNOUN
	"3" Num Arab Sg Nom <W:0.0000000000> @HNOUN
	"3" Num Arab Sg Ill Attr <W:0.0000000000> @HNOUN
"<,>"
	"," CLB <W:0.0000000000> <NoSpaceAfterPunctMark>
"<ja>"
	"ja" CC <W:0.0000000000> @CNP
#+end_src
...
#+begin_src cg
ADD (&no-space-after-punct-mark &LINK) TARGET (*) IF (-1 (<NoSpaceAfterPunctMark>));
ADDRELATION (RIGHT) (&no-space-after-punct-mark) TO (1 (&LINK) LINK 0 (&no-space-after-punct-mark)) ;
#+end_src

(The =&LINK= says that the following word is just a link, not the
central one, which matters for e.g. error messages, and the =RIGHT=
relation says that this is one big error, not two separate ones.)

Then we can add a suggestion that puts a space between the forms:
#+begin_src cg
COPY:no-space-after-punct ("<$1 $2>"v &SUGGESTWF) TARGET ("<(.*)>"r &no-space-after-punct-mark) IF (1 ("<(.*)>"r));
#+end_src
...
wordforms of the two cohorts. Now the output is
#+begin_src cg
"<3>"
	"3" Num Arab Sg Loc Attr <W:0.0000000000> @HNOUN
	"3" Num Arab Sg Nom <W:0.0000000000> @HNOUN
	"3" Num Arab Sg Ill Attr <W:0.0000000000> @HNOUN
"<,>"
	"," CLB <W:0.0000000000> <NoSpaceAfterPunctMark> &no-space-after-punct-mark ID:3 R:RIGHT:4 ADD:13066 ADDRELATION(RIGHT):13068
	"," CLB <W:0.0000000000> <NoSpaceAfterPunctMark> "<, ja>" &no-space-after-punct-mark &SUGGESTWF ID:3 R:RIGHT:4 ADD:13066 ADDRELATION(RIGHT):13068 COPY:13069:no-space-after-punct
"<ja>"
	"ja" CC <W:0.0000000000> @CNP &LINK &no-space-after-punct-mark ID:4 ADD:13067
#+end_src
