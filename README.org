...
Unfortunately we need a checkout of vislcg3 at the moment, since
vislcg3 doesn't install all header files. Our makefiles expect it to
be in the folder third-party/cg3. To fetch and compile the code, do:

#+BEGIN_SRC sh
svn co http://visl.sdu.dk/svn/visl/tools/vislcg3/trunk third-party/cg3
cd third-party/cg3
./cmake.sh
make -j3
sudo make install
...
- [ ] link to vislcg3 without needing the source (vislcg3 has to make the functions we use available from cg3.h)
