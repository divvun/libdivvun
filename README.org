...
You can pass =--enable-cgspell= to configure to get an error if any
=divvun-cgspell= dependencies are missing.
...
- [X] load a zhfst bin
- [ ] optionally load errmodel.hfst and acceptor.hfst instead of zhfst
